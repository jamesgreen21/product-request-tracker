...
def add_request():
    if request.method == 'POST':
        ...
        try:
            ...
            new_request["new_product"] = True if new == "on" else False
...
def edit_request(post_id):
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            post.ex_case_orientation = True if ex_case_orientation == "on" else False
        except:
        ...
        try:
            ...
            post.case_orientation = True if case_orientation == "on" else False
        except:
...
def output_form(post_id):
    post = Posts.query.get_or_404(post_id)
    health_and_safety = Actions.query.filter(Actions.stage == 1, Actions.posts_id == post_id).order_by(Actions.created_on.desc()).first()
    quality = Actions.query.filter(Actions.stage == 2, Actions.posts_id == post_id).order_by(Actions.created_on.desc()).first()
    cagefill = Actions.query.filter(Actions.stage == 3, Actions.posts_id == post_id).order_by(Actions.created_on.desc()).first()
    restaurant_impact = Actions.query.filter(Actions.stage == 4, Actions.posts_id == post_id).order_by(Actions.created_on.desc()).first()
...
def complete_request(post_id, status):
    ...
    if post.healthandsafety in (0, 2):
        flash("Please complete Health and Safety for {}.".format(post.title), 'error')
        return redirect(url_for('main.index'))
